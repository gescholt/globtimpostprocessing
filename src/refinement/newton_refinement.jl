...
- `eigenvectors::Matrix{Float64}`: Hessian eigenvectors (columns) at the refined point (0×0 if skipped)
- `condition_number::Float64`: Ratio max(|λ|)/min(|λ|) of Hessian eigenvalues (NaN if skipped/degenerate)
- `min_eigenvalue::Float64`: Smallest eigenvalue — indicates "strength" of minimum (NaN if skipped)

# Fields — Convergence diagnostics
...
# Fields — Trust-region diagnostics
...
- `trace_gradient_norms::Vector{Float64}`: Per-iteration ||∇f|| values (empty unless `trace=true`)

# Fields — Method metadata
- `gradient_method::Symbol`: Differentiation backend used (`:finitediff`, `:forwarddiff`, or `:unknown`)
"""
Base.@kwdef struct CriticalPointRefinementResult
    # Critical point
    point::Vector{Float64}
    objective_value::Float64 = NaN
    cp_type::Symbol = :unknown
    eigenvalues::Vector{Float64} = Float64[]
    eigenvectors::Matrix{Float64} = Matrix{Float64}(undef, 0, 0)
    condition_number::Float64 = NaN
    min_eigenvalue::Float64 = NaN
    # Convergence diagnostics
    gradient_norm::Float64 = Inf
    converged::Bool = false
    iterations::Int = 0
    initial_gradient_norm::Float64 = Inf
    # Trust-region diagnostics
    final_trust_radius::Float64 = NaN
    rejected_steps::Int = 0
    f_safeguard_count::Int = 0
    # Trace
    trace::Vector{Vector{Float64}} = Vector{Float64}[]
    trace_gradient_norms::Vector{Float64} = Float64[]
    # Method metadata
    gradient_method::Symbol = :unknown
end
...
Refine a raw polynomial critical point to a true critical point of `f` via trust-region
Newton's method.

Two modes are available:
- `mode = :critical_point` (default): Solve ∇f(x) = 0 — finds critical points of ALL types
  (minima, maxima, saddle points). Newton step uses the true Hessian eigenvalues.
- `mode = :minimize`: Modified Newton — replaces each Hessian eigenvalue λ with |λ| in the
  Newton step, making the effective Hessian positive-definite so every step is a descent
  direction. Quality ratio ρ is measured by objective decrease instead of gradient norm
  reduction. This avoids convergence to saddle points at the cost of not finding maxima.
...
and shrinks when steps are unproductive. An objective-value safeguard prevents steps that
jump to flat plateaus far from the current basin.
...
- `mode::Symbol`: `:critical_point` (default) solves ∇f = 0 to find any critical point type;
  `:minimize` uses modified Newton (|λ| eigenvalues) to only find local minima.
...
function refine_to_critical_point(
    ...
    mode in (:critical_point, :minimize) ||
        error("Unknown mode: $mode. Use :critical_point or :minimize")

    n = length(initial_point)
    x = copy(initial_point)

    # Trajectory recording: collect accepted iterates and gradient norms when trace=true
    trace_points = trace ? [copy(x)] : Vector{Float64}[]
    trace_grad_norms = Float64[]

    # Unpack bounds for internal clamping
    lb, ub = split_bounds(bounds)

    # Validate trust-region parameters
    0.0 < trust_radius_fraction <= 1.0 ||
        error("trust_radius_fraction must be in (0, 1], got $trust_radius_fraction")
    ...
    max_f_increase_factor > 1.0 ||
        error("max_f_increase_factor must be > 1, got $max_f_increase_factor")
    ...
    trace && push!(trace_grad_norms, grad_norm)  # seed with initial gradient norm
    ...
    while !converged && iterations < max_iterations
        ...
        # In :minimize mode, use |λ_i| instead of λ_i (modified Newton) so the
        # effective Hessian is PD and the step is always a descent direction.
        Vt_grad = V' * grad
        step = zeros(n)
        for i in 1:n
            if abs(eigenvalues[i]) > reg_threshold
                λ_eff = mode == :minimize ? abs(eigenvalues[i]) : eigenvalues[i]
                step .-= (Vt_grad[i] / λ_eff) .* V[:, i]
            end
        end
        ...
        # Quality ratio ρ: measures agreement between model and reality.
        # - :critical_point mode: ρ = (actual ||∇f||² reduction) / (predicted ||∇f||² reduction)
        # - :minimize mode: ρ = (actual f decrease) / (predicted f decrease from quadratic model)
        ρ = if mode == :minimize
            # Quadratic model prediction: f(x+s) ≈ f(x) + g'·s + 0.5·s'·H·s
            # Predicted decrease: -(g'·s + 0.5·s'·H·s)
            Hs = H_sym * step
            pred = -(LinearAlgebra.dot(grad, step) + 0.5 * LinearAlgebra.dot(step, Hs))
            actual = f_current - f_trial
            if abs(pred) < 1e-30
                actual > 0 ? 1.0 : 0.0
            else
                actual / pred
            ...
            predicted_reduction =
                grad_norm^2 - LinearAlgebra.norm(predicted_grad_after)^2
            if abs(predicted_reduction) < 1e-30
        ...
        if ρ > 0
            ...
            converged = grad_norm < tol
            if trace
                push!(trace_points, copy(x))
                push!(trace_grad_norms, grad_norm)
            end
        else
    ...
    cp_type = :unknown
    eigenvalues = Float64[]
    eigvecs = Matrix{Float64}(undef, 0, 0)
    cond_num = NaN
    min_eig = NaN
    ...
    if is_useful
        ...
        if all(isfinite, H_final)
            eig = LinearAlgebra.eigen(LinearAlgebra.Symmetric(H_final))
            eigenvalues = collect(eig.values)
            eigvecs = eig.vectors
            cp_type = _classify_eigenvalues(
                eigenvalues,
                hessian_tol;
                relative_tol = hessian_relative_tol,
            )
            cond_num, min_eig = _eigenvalue_diagnostics(eigenvalues)
        end
    end
    ...
    return CriticalPointRefinementResult(
        ...
        f_safeguard_count = f_safeguard_count,
        trace = trace_points,
        trace_gradient_norms = trace_grad_norms,
        gradient_method = gradient_method,
    )
end
...
function refine_to_critical_points(
    ...
    for (i, pt) in enumerate(points)
        t_start = time()
        results[i] = refine_to_critical_point(
            objective,
            pt;
        ...
        if is_useful
            ...
            f_str =
                is_f_accepted && !r.converged && r.gradient_norm >= accept_tol ?
                @sprintf("  f<%.0e", f_accept_tol) : ""
            @printf(
                io,
                "    ✓ CP %d/%d: %s %3d iters%s (%4.1fs)  |∇f| %.2e  f=%.2e  Δ=%.2e  (%s)%s\n",
                i,
                n_pts,
                status,
                r.iterations,
        ...
        if !is_useful && (i % 20 == 0 || i == n_pts)
            ...
            @printf(
                io,
                "    Refining: %d/%d (%d%%) | %d conv, %d acc, %d rej | %d TR rej%s\r",
                i,
                n_pts,
                pct,
                n_conv,
    ...
    n_f_accepted_final = count(
        r ->
            !r.converged &&
                r.gradient_norm >= accept_tol &&
                f_accept_tol !== nothing &&
                r.objective_value < f_accept_tol,
        results,
    )
    ...
    if f_accept_tol !== nothing && n_f_accepted_final > 0
        @printf(
            io,
            "    Summary: %d converged, %d grad-accepted, %d f-accepted (f<%.0e), %d rejected  (%.1fs)\n",
            n_conv,
            n_grad_accepted,
            n_f_accepted_final,
            f_accept_tol,
        ...
        @printf(
            io,
            "    Summary: %d converged, %d accepted, %d rejected  (%.1fs)\n",
            n_conv,
            n_accepted,
            n_rejected,
            total_time
    ...
    if total_tr_rejections > 0 || total_f_safeguards > 0
        @printf(
            io,
            "    Trust-region: %d step rejections, %d f-safeguards across %d CPs\n",
            total_tr_rejections,
            total_f_safeguards,
            n_pts
        )
...
"""
    _eigenvalue_diagnostics(eigenvalues) -> (condition_number, min_eigenvalue)

Compute condition number and minimum eigenvalue from a vector of Hessian eigenvalues.
Returns `(NaN, NaN)` for empty eigenvalue vectors.
"""
function _eigenvalue_diagnostics(eigenvalues::Vector{Float64})
    if isempty(eigenvalues)
        return (NaN, NaN)
    end
    abs_eigs = abs.(eigenvalues)
    min_abs = minimum(abs_eigs)
    max_abs = maximum(abs_eigs)
    cond = min_abs > 0 ? max_abs / min_abs : Inf
    return (cond, minimum(eigenvalues))
end
...
Convert an Optim.jl `RefinementResult` (from `core_refinement.jl`) into a
`CriticalPointRefinementResult` so both refinement paths produce the same type
for downstream consumers. Works for all Optim methods (NelderMead, LBFGS, BFGS, etc.).
...
"""
function _wrap_optim_as_cpresult(
    r::RefinementResult,
    objective;
    gradient_method::Symbol = :finitediff,
    hessian_tol::Float64 = 1e-6,
    ...
    elseif gradient_method == :finitediff
        ...
        error(
            "Unknown gradient_method: $gradient_method (expected :forwarddiff or :finitediff)",
        )
    end
    ...
    # Compute Hessian eigenvalues + eigenvectors for CP classification
    cp_type = :unknown
    eigenvalues = Float64[]
    eigvecs = Matrix{Float64}(undef, 0, 0)
    cond_num = NaN
    ...
    result_trace = if !isempty(r.trace)
        r.trace
    ...
    return CriticalPointRefinementResult(
        point = point,
        objective_value = r.value_refined,
        cp_type = cp_type,
        eigenvalues = eigenvalues,
        eigenvectors = eigvecs,
        condition_number = cond_num,
...
"""
    classify_eigenvalues(eigenvalues, tol; relative_tol=0.0) -> Symbol
...
# Arguments
- `eigenvalues`: Hessian eigenvalues (sorted or unsorted)
...
# Example
```julia
classify_eigenvalues([2.0, 3.0], 1e-6)           # → :min
classify_eigenvalues([2.0, -1.0], 1e-6)          # → :saddle
classify_eigenvalues([2.0, 1e-10], 1e-6)         # → :degenerate_min
classify_eigenvalues([2.0, 1e-10], 1e-6;
                     relative_tol=0.01)           # → :degenerate_min (adaptive tol)
```
"""
function classify_eigenvalues(
...
# Internal alias — keeps existing private callers working during transition
const _classify_eigenvalues = classify_eigenvalues
