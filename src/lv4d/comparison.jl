...
function load_comparison_data(path::String)::ComparisonData
    ...
    isempty(missing_cols) ||
        error("Missing required columns in comparison_results.csv: $missing_cols")
...
"""
function find_comparison_experiments(
...
function _print_overall_summary(df::DataFrame, metric_col::Symbol)
    ...
    metric_label =
        metric_col == :best_dist_to_true ? "dist_to_true(med)" : "$(metric_col)(med)"
...
function _print_per_domain_summary(df::DataFrame, dom::Float64, metric_col::Symbol)
    ...
    hl_loser = make_loser_highlighter(
        (data, i, j) ->
            haskey(degree_winners, data[i, 2]) &&
...
function _print_ratio_histogram(df::DataFrame, metric_col::Symbol)
    ...
    title_str = @sprintf(
        "n=%d | log wins: %d | std wins: %d | median=%.2f",
        length(ratios),
        n_log_better,
        n_std_better,
        median(ratios)
...
function prepare_subdivision_comparison_df(
    ...
    for (key, (single_exp, subdiv_exp)) in matched
        ...
        for (method_name, exp) in [("single", single_exp), ("subdivision", subdiv_exp)]
            ...
            if isempty(deg_rows)
                error(
                    "No results for degree $degree in experiment $(experiment_id(exp)). Available degrees: $(unique(dr.degree))",
                )
            end
...
"""
function print_subdivision_comparison(
    df::DataFrame;
    io::IO = stdout,
    show_aggregated::Bool = true,
)
    if isempty(df)
    ...
    display_df = DataFrame(
        ...
        single_recovery = [
            isnan(v) ? "-" : @sprintf("%.1f%%", v * 100) for v in wide_df.single_recovery
        ],
        subdiv_recovery = [
            isnan(v) ? "-" : @sprintf("%.1f%%", v * 100) for v in wide_df.subdiv_recovery
        ],
    ...
    hl_better = Highlighter(
        (data, i, j) -> l2_highlight(data, i, j) || recovery_highlight(data, i, j),
        bold = true,
        foreground = :green,
    )
    ...
    if show_aggregated && length(unique(wide_df.seed)) > 1
        ...
        agg_df = combine(
            groupby(wide_df, [:GN, :degree, :domain]),
        ...
        agg_display = DataFrame(
            ...
            single_rec = [
                isnan(v) ? "-" : @sprintf("%.1f%%", v * 100) for v in agg_df.single_rec_mean
            ],
            subdiv_rec = [
                isnan(v) ? "-" : @sprintf("%.1f%%", v * 100) for v in agg_df.subdiv_rec_mean
            ],
        )
...
function compare_single_vs_subdivision(
    ...
    if domain !== nothing
        experiments =
            filter(e -> isapprox(e.params.domain, domain; rtol = 0.01), experiments)
    end
    if seed !== nothing
    ...
    if isempty(matched)
        ...
        println(
            io,
            "experiments exist for the same (GN, degree, domain, seed) combinations.",
        )
        return DataFrame()
    end
    ...
    if !isempty(single_metrics) && !isempty(subdiv_metrics)
        ...
        @printf(
            io,
            "  L2 median: single=%.2e, subdiv=%.2e (ratio=%.2f, %s better)\n",
            single_l2_med,
            subdiv_l2_med,
            l2_ratio,
            l2_winner
        ...
        if !isempty(single_rec) && !isempty(subdiv_rec)
            ...
            @printf(
                io,
                "  Recovery median: single=%.1f%%, subdiv=%.1f%% (ratio=%.2f, %s better)\n",
                single_rec_med * 100,
                subdiv_rec_med * 100,
                rec_ratio,
                rec_winner
        ...
        @printf(
            io,
            "  Total critical points: single=%d, subdiv=%d\n",
            single_cps,
            subdiv_cps
        )
        ...
        @printf(
            io,
            "  Total Hessian minima: single=%d, subdiv=%d\n",
            single_minima,
            subdiv_minima
        )
    end
