...
"""
function _tui_detect_degree_values(
...
"""
function _tui_detect_domain_values(
    results_root::Union{String,Nothing};
    gn::Union{Int,Nothing} = nothing,
)::Vector{Float64}
    experiments = find_experiments(results_root)
...
function _tui_select_degree_range(
    results_root::Union{String,Nothing};
    gn::Union{Int,Nothing} = nothing,
...
function _tui_select_domain_filter(
    results_root::Union{String,Nothing};
    gn::Union{Int,Nothing} = nothing,
...
function _tui_select_comparison_experiment(
    results_root::Union{String,Nothing},
...
function _tui_run_sweep(results_root::Union{String,Nothing})
    ...
    println(
        "$(TUI_GREEN)✓$(TUI_RESET) GN=$(TUI_BOLD)$gn$(TUI_RESET), degree=$(TUI_BOLD)$deg_str$(TUI_RESET), domain≤$(TUI_BOLD)$domain_max$(TUI_RESET)",
    )
    println("$(TUI_DIM)────────────────────────────────────$(TUI_RESET)")
    println()
...
function _tui_run_quality(results_root::Union{String,Nothing})
    ...
    println("$(TUI_DIM)────────────────────────────────────$(TUI_RESET)")
    println(
        "$(TUI_GREEN)✓$(TUI_RESET) Experiment: $(TUI_BOLD)$(basename(exp_dir))$(TUI_RESET)",
    )
    println("$(TUI_DIM)────────────────────────────────────$(TUI_RESET)")
    println()
...
function _tui_run_convergence(results_root::Union{String,Nothing})
    ...
    println(
        "$(TUI_GREEN)✓$(TUI_RESET) GN=$(TUI_BOLD)$gn$(TUI_RESET), degree=$(TUI_BOLD)$deg_str$(TUI_RESET)$csv_str",
    )
    println("$(TUI_DIM)────────────────────────────────────$(TUI_RESET)")
    println()
...
function _tui_run_comparison(results_root::Union{String,Nothing})
    ...
    println("$(TUI_DIM)────────────────────────────────────$(TUI_RESET)")
    println(
        "$(TUI_GREEN)✓$(TUI_RESET) Comparison: $(TUI_BOLD)$(basename(comp_dir))$(TUI_RESET)",
    )
    println("$(TUI_DIM)────────────────────────────────────$(TUI_RESET)")
    println()
...
function _tui_run_subdivision(results_root::Union{String,Nothing})
    ...
    domain !== nothing &&
        push!(filter_parts, "domain=$(TUI_BOLD)$(@sprintf("%.4f", domain))$(TUI_RESET)")
...
"""
function _tui_select_domain_value(
    results_root::Union{String,Nothing};
    gn::Union{Int,Nothing} = nothing,
)::Union{Float64,Nothing}
    domains = _tui_detect_domain_values(results_root; gn = gn)
...
function _tui_run_coverage(results_root::Union{String,Nothing})
    ...
    println(
        "$(TUI_GREEN)✓$(TUI_RESET) GN=$(TUI_BOLD)$gn_values$(TUI_RESET), domains=$(TUI_BOLD)$domain_values$(TUI_RESET)",
    )
    println(
        "  degrees=$(TUI_BOLD)$deg_str$(TUI_RESET), seeds=$(TUI_BOLD)$seed_str$(TUI_RESET)",
    )
    println("$(TUI_DIM)────────────────────────────────────$(TUI_RESET)")
    println()
    ...
    if !isempty(report.missing_keys)
        ...
        if generate
            ...
            if output_dir !== nothing
                ...
                println()
                println(
                    "$(TUI_GREEN)Generated $(length(configs)) config files in $(TUI_BOLD)$output_dir$(TUI_RESET)",
                )
            end
        end
    end
...
function _tui_select_seed_range()::Vector{Int}
    options = ["1:5 (5 seeds)", "1:3 (3 seeds)", "1 only (single seed)", "Custom range"]
    menu = RadioMenu(options, pagesize = 4)
...
function _tui_select_output_directory()::Union{String,Nothing}
    ...
    options =
        ["experiments/fill_gaps/ (default)", "experiments/generated/", "Custom directory"]
