...
function analyze_convergence(
    ...
    degree_min =
        filter.degree isa SweepRange ? filter.degree.min :
        filter.degree isa FixedValue ? filter.degree.value : 4
    degree_max =
        filter.degree isa SweepRange ? filter.degree.max :
...
function _collect_convergence_data(
    results_root::Union{String,Nothing};
    gn::Int,
    degree::Int,
)
...
function _print_convergence_summary(
...
function _compute_convergence_rate(data::Dict{Float64,Vector{Float64}})
    ...
    n = length(log_d)
    slope =
        (n * sum(log_d .* log_r) - sum(log_d) * sum(log_r)) /
...
function _print_convergence_table(all_slopes::Dict{Int,Float64})
    ...
    display_df =
        DataFrame("Deg" => deg_col, "α (rate)" => rate_col, "Interpretation" => interp_col)
...
"""
function _export_convergence_analysis_csv(
    results_root::String,
    all_data::Dict{Int,Dict{Float64,Vector{Float64}}},
    all_l2_data::Dict{Int,Dict{Float64,Vector{Float64}}},
    gn::Int,
