...
using LinearAlgebra: norm, dot, eigen, Symmetric, cond, pinv, eigvals
using Printf: @printf

# Gradient/Hessian dispatch — uses ForwardDiff or FiniteDiff depending on config.
# ForwardDiff gives machine-precision derivatives but requires the objective to accept
# Dual numbers (i.e. generic Real types). ODE-based objectives that mutate pre-allocated
# Float64 buffers (e.g. Dynamic_objectives) are NOT ForwardDiff-compatible and must use
# FiniteDiff. See data_generation.jl:69 for the known blocker.

function _vw_gradient(f, x::AbstractVector, method::Symbol)
    if method == :forwarddiff
        return ForwardDiff.gradient(f, x)
    elseif method == :finitediff
        return FiniteDiff.finite_difference_gradient(f, x)
    else
        error("Unknown gradient_method: $method. Use :forwarddiff or :finitediff")
    end
end

function _vw_hessian(f, x::AbstractVector, method::Symbol)
    if method == :forwarddiff
        return ForwardDiff.hessian(f, x)
    elseif method == :finitediff
        return FiniteDiff.finite_difference_hessian(f, x)
    else
        error("Unknown gradient_method: $method. Use :forwarddiff or :finitediff")
    end
end
...
- `gradient_method`: Differentiation backend — `:forwarddiff` or `:finitediff` (default: :forwarddiff).
  Use `:finitediff` for ODE-based objectives that are not ForwardDiff-compatible.
"""
...
function detect_valley(f, point::AbstractVector, config::ValleyWalkConfig)
    grad = _vw_gradient(f, point, config.gradient_method)
    hess = _vw_hessian(f, point, config.gradient_method)
...
"""
    project_to_valley(f, point; max_iter=10, tol=1e-10, gradient_method=:forwarddiff) -> Vector
...
function project_to_valley(
    ...
    for _ in 1:max_iter
        g = _vw_gradient(f, x, gradient_method)
        norm(g) < tol && break
        H = _vw_hessian(f, x, gradient_method)
...
"""
    project_to_valley_tangent(f, point, tangent; max_iter=10, tol=1e-10, gradient_method=:forwarddiff) -> Vector
...
"""
function get_valley_tangent(
    f,
    point::AbstractVector,
    prev_dir::AbstractVector,
...
"""
function walk_newton_projection(
    f,
    start_point::AbstractVector,
    initial_direction::AbstractVector,
    config::ValleyWalkConfig,
)
...
"""
function walk_predictor_corrector(
    f,
    start_point::AbstractVector,
    initial_direction::AbstractVector,
    config::ValleyWalkConfig,
...
"""
function walk_tangent_projection(
    f,
    start_point::AbstractVector,
    initial_direction::AbstractVector,
    config::ValleyWalkConfig,
    ...
    for _ in 1:config.max_steps
        ...
        projected = project_to_valley_tangent(
            f,
            candidate,
            direction;
            max_iter = config.max_projection_iter,
...
function trace_valley(
    ...
    elseif config.method == :newton_projection
        ...
        error(
            "Unknown valley walking method: $(config.method). " *
    ...
    # Compute arc length
    arc_len =
        sum(norm(path_pos[i+1] - path_pos[i]) for i in 1:(length(path_pos)-1); init = 0.0) +
...
"""
function trace_valleys_from_critical_points(
    f,
    df::DataFrame,
    config::ValleyWalkConfig = ValleyWalkConfig(),
)
...
function run_valley_analysis(
    ...
    println(
        "  $n_degen degenerate CP$(n_degen > 1 ? "s" : "") detected (of $n_total total)",
    )
    println()
    ...
    for (i, cp) in enumerate(degenerate_cps)
        ...
        @printf(
            "  CP %2d: dim=%d  arc_length=%.2f  points=%d  %s\n",
            cp_idx,
            trace.valley_dimension,
            trace.arc_length,
            trace.n_points,
            status
        )
    ...
    if n_traced > 0
        ...
        println(
            "  No valleys could be traced (CPs may not lie exactly on a valley manifold)",
        )
    end
    println()
