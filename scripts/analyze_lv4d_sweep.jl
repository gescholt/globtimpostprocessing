...
function parse_experiment_name(dirname::String)
    ...
    m = match(
        r"lv4d_GN(\d+)_deg(\d+)(?:-(\d+))?_dom(?:ain)?([\d.eE+-]+)_seed(\d+)_",
        dirname,
    )
    if m !== nothing
...
function load_critical_point_metrics(exp_dir::String)
    ...
    csv_files = filter(
        f -> startswith(f, "critical_points_deg_") && endswith(f, ".csv"),
        readdir(exp_dir),
    )
...
function analyze_sweep(results_dir::String)
    ...
    exp_dirs = if is_single_experiment(results_dir)
        ...
        filter(
            d -> startswith(basename(d), "lv4d_") && isdir(d),
    ...
    for (i, row) in enumerate(eachrow(summary))
        ...
        data[i, 6] =
            isnan(row.mean_recovery) ? "-" : @sprintf("%.2f%%", row.mean_recovery * 100)
    ...
    if nrow(valid_l2) > 0
        ...
        println()
        @printf(
            "Best: GN=%d dom=%.1e deg=%d → L2=%.1f RecErr=%.2f%% Succ=%.0f%%\n",
            best.GN,
            best.domain,
            best.degree,
            best.mean_L2,
...
function main()
    ...
    results_dir = if !isempty(args)
        ...
        results_root = get(
            ...
            joinpath(globoptim_root, "globtim_results"),
        )
