...
# Note: This dataset is generated by HPC runs and may not be present locally
const TEST_DATASET_ROOT = joinpath(
    dirname(@__DIR__),
    "..",
    "globtim",  # Active directory (globtim is archived)
    "experiments",
    "daisy_ex3_4d_study",
    ...
    if !isdir(TEST_DATASET_ROOT)
        ...
        @testset "Load critical points from real experiment" begin
            ...
            if !isempty(exp_dirs)
                ...
                csv_files = filter(
                    f -> occursin(r"critical_points_deg_\d+\.csv", f),
                    readdir(exp_path),
                )
            ...
            if !isempty(exp_dirs)
                ...
                if haskey(config, "p_true")
                    ...
                    if !isempty(csv_files)
                        ...
                        println(
                            "      Min distance:  $(round(stats["min_distance"], digits=6))",
                        )
                        println(
                            "      Mean distance: $(round(stats["mean_distance"], digits=6))",
                        )
                        println(
                            "      Recoveries:    $(stats["num_recoveries"]) / $(nrow(df))",
                        )
                    end
                end
            end
                ...
                if haskey(config, "p_true")
                    ...
                    if length(degrees) >= 2
                        ...
                        for row in eachrow(recovery_table)
                            println(
                                "    Degree $(row.degree): $(row.num_critical_points) CPs, " *
