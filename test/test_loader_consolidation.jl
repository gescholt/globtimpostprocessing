...
function write_config(dir; p_true = [1.0, 1.5])
    ...
    write(
        path,
        """{"p_true": $(p_true), "dimension": $(length(p_true)), "function_name": "test"}""",
    )
    return path
end
...
@testset "Loader consolidation" begin
    ...
    @testset "load_critical_points_from_csvs: delegates to _load_critical_points" begin
        mktempdir() do dir
            ...
            result = GlobtimPostProcessing.load_critical_points_from_csvs(
                dir,
                Dict{String,Any}(),
            )
            @test result !== nothing
    ...
    @testset "print_parameter_recovery_table: runs without error on _raw_ fixture" begin
        mktempdir() do dir
            ...
            cp(
                joinpath(fixtures_dir, "critical_points_raw_deg_4.csv"),
            ...
            campaign = CampaignResults("test_campaign", [exp], Dict{String,Any}(), now())
    ...
    @testset "print_parameter_recovery_table: skips experiment without p_true" begin
        mktempdir() do dir
            ...
            write(
                joinpath(dir, "experiment_config.json"),
