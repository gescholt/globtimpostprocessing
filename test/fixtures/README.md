...
This directory contains realistic test data generated by globtim using the Deuflhard_4d benchmark function.
...
To regenerate test fixtures with the latest globtim version:
...
**When to regenerate:**
- After changing globtim's CSV export format
- After updating critical point filtering logic
- When Phase 2 schema changes
- To update with latest globtim algorithms
...
| **Data source** | Hand-crafted | Real globtim output |
...
- `globtim/test/fixtures/generate_postprocessing_fixtures.jl` - Generator script
- `globtim/src/LibFunctions.jl` - Deuflhard_4d function definition
- `globtim/test/validation/deuflhard_4d_minimal.jl` - Usage example
